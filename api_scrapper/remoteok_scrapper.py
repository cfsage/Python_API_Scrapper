...
def output_jobs_to_xls(data):
    wb = Workbook()
    job_sheet = wb.add_sheet('RemoteOK Jobs')
    headers = data[0].keys()
    ...
    for i in range(0, len(data)):
        ...
        for x in range (0, len(values)):
            # using i + 1 cause 0 is used as the header row
            job_sheet.write(i + 1, x, (values[x]))
            
    wb.save('remoteok_jobs.xls')
    
    # sending the mail with the generated xls file 
def send_email(send_from, send_to, subject, text, files=None,
    ...
    smtp.starttls()
    smtp.login(send_from, 'password')  # Replace with your email password
...
if __name__ == "__main__":
    ...
    send_email('sender_mail',['reciever_mail'], 'Jobs Posting', 
               'Please find attached the latest RemoteOK job postings.', 
